...
class ks_general_ledger(models.AbstractModel):
    ...
    def _get_templates(self):
        templates = super(ks_general_ledger, self)._get_templates()
        ...
        for account in sorted_accounts:
            ...
            if 'account_%s' % (account.id,) in options.get('unfolded_lines') or unfold_all:
                ...
                for line in amls:
                    ...
                    ks_line_odoorates = line.company_id.currency_id._get_rates(company_id, date)
                    ks_line_odoorate = ks_line_odoorates.get(line.company_id.currency_id) or 1.0
                    ks_line_rate = 1/ks_line_odoorate
                    progress = progress + line_debit - line_credit
                    currency = "" if not line.currency_id else self.with_context(no_format=False).format_value(line.amount_currency, currency=line.currency_id) or line.company_id.currency_id.name
                    ...
                    columns[4]['class'] = 'whitespace_print'
                    columns[3]['title'] = name_title
                    columns[4]['title'] = partner_name_title
                    line_value = {
                        'id': line.id,
